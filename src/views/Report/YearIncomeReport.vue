	...
	export default {
		...
		watch: {
			yearRadio(val, oldval) {
				this.yearRadio = val
				incomeYear(val).then(res => {
					this.drawLine(res,this.types);
				})
			},
			types(val, oldval) {
				incomeYear(this.yearRadio).then(res => {
					this.drawLine(res,val);
				})
			},
		},
		computed: {
		...
		async mounted() {
			...
			this.dataToday = await incomeYear(new Date().getFullYear())
